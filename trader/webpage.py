...
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  .toast {
    ...
    transform: translate(-50%, 12px); z-index: 50; max-width: min(92vw, 460px);
    padding: 12px 16px; border-radius: 14px; font-size: 14px; font-weight: 600;
    line-height: 1.35; color: var(--ink); background: var(--card-solid);
    border: 1px solid var(--ring); box-shadow: 0 14px 40px -12px rgba(0,0,0,0.45);
    opacity: 0; pointer-events: none; transition: opacity .18s ease, transform .18s ease;
  }
  .toast.show { opacity: 1; transform: translate(-50%, 0); }
  .toast.ok  { border-color: color-mix(in srgb, var(--accent) 55%, var(--ring)); }
  .toast.err { border-color: color-mix(in srgb, var(--s3, #e5484d) 60%, var(--ring)); }
...
// ---- refrescar el ranking (lanza el recálculo en GitHub Actions) ----
(() => {
  ...
  const toast = (msg, kind, ms) => {
    ...
    if (ms) toastTimer = setTimeout(() => { toastEl.className = "toast"; }, ms);
  };

  // owner/repo se deducen del propio dominio (GitHub Pages: owner.github.io/repo)
  const detectRepo = () => {
    let owner = "fedegarlo", repo = "trader";
  ...
  const reset = () => { btn.classList.remove("spin"); btn.disabled = false; };
  ...
  btn.addEventListener("click", async () => {
    ...
    if (!token) {
      token = window.prompt(
        "Para recalcular el ranking hace falta tu token de GitHub " +
        "(fine-grained, con permiso Contents sobre el repo). " +
        "Es el mismo que usas en la página de subir extractos:");
      if (!token) return;
      token = token.trim();
      if (!token) return;
    }
    ...
    btn.disabled = true;
    toast("Lanzando el recálculo del ranking…", "");
    ...
    try {
      const res = await fetch(
        ...
        body: JSON.stringify({ event_type: "recalcular-ranking" }),
      });
      if (res.status === 204) {
        toast("Recálculo en marcha ✓ · la clasificación se actualizará en 1–2 min. " +
          "Recargaré sola cuando esté.", "ok");
        // el workflow tarda ~1–2 min; recargamos pasado ese margen (sin caché)
        setTimeout(() => location.replace(location.pathname + "?r=" + Date.now()), 120000);
      } else if (res.status === 401 || res.status === 403) {
        toast("Token inválido o sin permiso <b>Contents</b> sobre el repo. " +
          "Revísalo en la página de subir extractos.", "err", 8000);
        reset(); busy = false;
      } else {
        toast("No se pudo lanzar el recálculo (HTTP " + res.status + ").", "err", 8000);
        reset(); busy = false;
      }
    } catch (e) {
      toast("Error de red al lanzar el recálculo. Inténtalo de nuevo.", "err", 8000);
      reset(); busy = false;
    }
  });
})();
