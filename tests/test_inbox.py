...
def test_keep_unread_only_when_report_missing():
    ...
    assert inbox.Result("no_csv").keep_unread
    assert inbox.Result("invalid_csv").keep_unread
...
def test_run_does_not_mark_as_read_without_report(monkeypatch, tmp_path):
    ...
    assert [r.status for r in summary.skipped] == ["no_csv"]
...
def test_run_does_not_mark_as_read_with_unreadable_report(monkeypatch, tmp_path):
    ...
    assert [r.status for r in summary.skipped] == ["invalid_csv"]
    assert fake.stored == []
